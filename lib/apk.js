...
const { findApkAbis } = require('./abi')
const { parseResourceConfigs } = require('./resource-config')
...
class ApkParser extends Zip {
  ...
  async parse () {
    const [buffers, entryNames] = await Promise.all([
    ...
    apkInfo.abis = findApkAbis(entryNames)
    apkInfo.resourceConfigs = {
      locales: [],
      densities: [],
      sdkVersions: [],
      configurations: []
    ...
    apkInfo.resourceConfigs = parseResourceConfigs(buffers[ResourceName])
