...
const TypedValue = {
  ...
  TYPE_DYNAMIC_REFERENCE: 0x00000007,
  TYPE_DYNAMIC_ATTRIBUTE: 0x00000008,
...
class BinaryXmlParser {
  ...
  readDimension () {
    ...
    const unit = (value >> TypedValue.COMPLEX_UNIT_SHIFT) & TypedValue.COMPLEX_UNIT_MASK

    dimension.value = this.complexToFloat(value)
  ...
  readFraction () {
    ...
    const value = this.readU32()
    const type = (value >> TypedValue.COMPLEX_UNIT_SHIFT) & TypedValue.COMPLEX_UNIT_MASK

    fraction.value = this.complexToFloat(value)
  ...
  readHex24 () {
    ...
    var val = ('000000' + (this.readU32() & 0xffffff).toString(16)).slice(-6)
    this.debug && console.groupEnd()
    return val
  }
  ...
  readHex32 () {
    ...
    var val = ('00000000' + this.readU32().toString(16)).slice(-8)
    this.debug && console.groupEnd()
    return val
  }
  ...
  readTypedValue () {
    ...
    switch (dataType) {
      ...
      case TypedValue.TYPE_STRING:
        ...
        typedValue.value = ref >= 0 && ref < this.strings.length ? this.strings[ref] : ''
      ...
      case TypedValue.TYPE_DYNAMIC_REFERENCE:
        ...
        typedValue.type = dataType === TypedValue.TYPE_DYNAMIC_REFERENCE ? 'dynamic_reference' : 'reference'
        break
      case TypedValue.TYPE_ATTRIBUTE:
      case TypedValue.TYPE_DYNAMIC_ATTRIBUTE:
        id = this.readU32()
        typedValue.value = `resourceId:0x${id.toString(16)}`
        typedValue.type = dataType === TypedValue.TYPE_DYNAMIC_ATTRIBUTE ? 'dynamic_attribute' : 'attribute'
        break
      case TypedValue.TYPE_FLOAT:
        typedValue.value = this.convertIntToFloat(this.readU32())
...
and is supposed to end at offset ${end}. Ignoring the rest of the value.`)
  ...
  convertIntToFloat (int) {
    ...
    view.setUint32(0, int >>> 0, true)
    return view.getFloat32(0, true)
  }

  // Equivalent to android.util.TypedValue.complexToFloat().
  complexToFloat (complex) {
    const mantissaMult = 1.0 / (1 << TypedValue.COMPLEX_MANTISSA_SHIFT)
    const radixMults = [
      1.0 * mantissaMult,
      1.0 / (1 << 7) * mantissaMult,
      1.0 / (1 << 15) * mantissaMult,
      1.0 / (1 << 23) * mantissaMult
    ]
    const mantissa = complex & (TypedValue.COMPLEX_MANTISSA_MASK << TypedValue.COMPLEX_MANTISSA_SHIFT)
    const radix = (complex >> TypedValue.COMPLEX_RADIX_SHIFT) & TypedValue.COMPLEX_RADIX_MASK
  ...
  readXmlAttribute () {
    ...
    // A string-pool index of 0 is valid. -1 means there is no raw value.
    // Do not mutate versionName: spaces, Unicode and punctuation are legal.
    if (valueRef >= 0) {
